...
TEST(REPORT_OK, DEV_NAME_ONLY_OK)
{
	int n = 0;
	int len = 0;
	int conndfd;
	...
	p_pack_buf = BP_PackReport(DEV_NAME, BlankSysSigMap, BP_NULL, sizeof(BlankSysSigMap) / sizeof(BP_SysSigMap));
	n=send(conndfd,p_pack_buf->PackStart,p_pack_buf->MsgSize,0);
	ASSERT_EQ(n, p_pack_buf->MsgSize);
